...
def complete_random_sample_technique(X_train,X_test):
    try:
        logger.info(f'Before :{X_train.shape}')
        logger.info(f'Before :{X_test.shape}')
        logger.info(f'Before :{X_train.columns}')
        logger.info(f'Before :{X_test.columns}')
        logger.info(f'Before :{X_train.isnull().sum()}')
        logger.info(f'Before :{X_test.isnull().sum()}')
        for i in X_train.columns:
            if X_train[i].isnull().sum() > 0:
                logger.info(f'Column {i} ')
        ...
        logger.info(f'After :{X_train.shape}')
        logger.info(f'After :{X_test.shape}')
        logger.info(f'After :{X_train.columns}')
        logger.info(f'After :{X_test.columns}')
        logger.info(f'After :{X_train.isnull().sum()}')
        logger.info(f'After :{X_test.isnull().sum()}')
